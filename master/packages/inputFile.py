...
def importInputFile():
    ...
    stock_mother_coil_df = pd.read_excel("./data/stock_mother_coil.xlsx")
    stock_mother_coil_df.fillna(0, inplace=True)
    # stock of offcuts of standard width
    stock_slit_coil_df = pd.read_excel("./data/stock_slit_coil.xlsx")
    stock_slit_coil_df.fillna(0, inplace=True)
    # customer request
    demand_df = pd.read_excel("./data/design.xlsx")

    # thickness of different coils (depends on the grade of material)
    thick_df = pd.read_excel("./data/thick.xlsx")

    # trim loss required (depends on the grade of material)
    trim_df = pd.read_excel("./data/trim.xlsx")
